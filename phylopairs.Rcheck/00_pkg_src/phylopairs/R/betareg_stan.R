...
#' @export
betareg_stan = function(des, y, link="logit", covmat=NULL, itnum=6000, chains=4, coef.u=0, 
  ...
  if(!is.null(covmat)) {
    ...
    fit = sampling(object = stanmodels$betamm, data = stan.dat, iter = itnum, chains = chains, cores=cores, ...)
    ...
    fit = sampling(object = stanmodels$betareg, data = stan.dat, iter = itnum, chains = chains,cores=cores, ...)
