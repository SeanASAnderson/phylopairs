...
// Cp is a phylogenetic covariance matrix if species data are being analyzed
// The model is Y = XB + u + e, where u~N(0, sig2c*C) and e~N(0, sig2i*I)
// This is equivalent to a GLS in which Y = XB + eps where eps~N(0,(sig2c*C + sig2i*I)
...
model {
  ...
  Y ~ normal(X*Coef + pair_effects, sigma_resid);
}
...
generated quantities {
  ...
  for (n in 1:N) {
    y_pred[n] = normal_rng(X[n] * Coef + pair_effects[n], sigma_resid);  // Generate predicted values
    loglik[n] = normal_lpdf(Y[n] | X[n]*Coef + pair_effects[n], sigma_resid);
  }
}
